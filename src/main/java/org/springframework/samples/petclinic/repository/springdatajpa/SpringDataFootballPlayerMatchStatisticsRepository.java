...
public interface SpringDataFootballPlayerMatchStatisticsRepository extends FootballPlayerMatchStatisticRepository, Repository<FootballPlayerMatchStatistic, Integer> {
	...
	Collection<FootballPlayerMatchStatistic> findAllFootballPlayerMatchStatisticsBySeason(@Param("season_start") String season_start, @Param("season_end") String season_end);

	@Override
	@Query("SELECT a FROM FootballPlayerMatchStatistic a WHERE a.matchRecord.id =:match_record_id")
	Collection<FootballPlayerMatchStatistic> findFootballPlayerMatchStatisticByMatchRecordId(@Param("match_record_id") int match_record_id);

	@Override
	@Query("SELECT a FROM FootballPlayerMatchStatistic a WHERE a.id =:id")
	FootballPlayerMatchStatistic findFootballPlayerMatchStatisticById(@Param("id") int id);

	@Override
	@Query("SELECT a FROM FootballPlayerMatchStatistic a WHERE a.player.id =:player_id")
	Collection<FootballPlayerMatchStatistic> findFootballPlayerMatchStatisticByPlayerId(@Param("player_id") int player_id);

	@Override
	@Query("SELECT a FROM FootballPlayerMatchStatistic a WHERE a.player.id =:playerId AND a.matchRecord.id =:matchRecordId")
	FootballPlayerMatchStatistic findFootballPlayerMatchStatisticByPlayerIdAndMatchRecordId(@Param("playerId") int playerId, @Param("matchRecordId") int matchRecordId);
