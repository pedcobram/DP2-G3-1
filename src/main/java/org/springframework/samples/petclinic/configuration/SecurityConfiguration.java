...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(final HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/resources/**", "/webjars/**", "/h2-console/**").permitAll().antMatchers(HttpMethod.GET, "/", "/exception/**").permitAll().antMatchers("/users/new").permitAll().antMatchers("/authenticateds/new").permitAll()
			...
			.hasAnyAuthority("president").antMatchers("/footballPlayer/new").hasAnyAuthority("president").antMatchers("/contractPlayer/**").hasAnyAuthority("president").antMatchers("/contractsCommercial/**").hasAnyAuthority("president")
			.antMatchers("/fan/**").authenticated().anyRequest().denyAll().and().formLogin()
			/* .loginPage("/login") */
