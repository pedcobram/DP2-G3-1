...
public class PresidentRequestE2ETests {
	...
	void rejectPresidentRequest() throws Exception {
		this.mockMvc.perform(MockMvcRequestBuilders.get("/presidentRequest/reject/{usuario}", "gonzalo")).andExpect(MockMvcResultMatchers.status().is3xxRedirection()).andExpect(MockMvcResultMatchers.view().name("redirect:/presidentRequest/list"));
	}
